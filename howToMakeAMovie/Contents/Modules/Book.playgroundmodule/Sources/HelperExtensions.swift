...
extension SCNMaterialProperty {
    func updateAppearance(traitCollection: UITraitCollection) {
        if let lightString = contents as? String,
            let lastLightSubstring = lightString.split(separator: "/").last {
            contents = UIImage(named: String(lastLightSubstring))
        } else if let lightURL = contents as? URL {
            contents = UIImage(named: lightURL.lastPathComponent)
        } else if let lightImage = contents as? UIImage {
            contents = lightImage.imageAsset?.image(with: traitCollection)
        }
    }
}
