...
final class Scene: NSObject {
    ...
    init(source: SCNSceneSource) throws {
        ...
        guard let baseGridNode = source.entry(withID: GridNodeName) else {
    ...
    func enterState(_ newState: State) {
        ...
        case .done:
            ...
            gridWorld.calculateRowColumnCount()
            gridWorld.spokeAboutRandomNodes = false
        }
    }
    ...
    func positionCameraToFitGrid(around node: SCNNode) {
        ...
        cameraNode.camera?.fov = CGFloat(2.0 * halfAngle * 180.0 / .pi)
    }
    ...
    /// Resets the scene in preparation for another run.
    ...
    func updateAppearance(traitCollection: UITraitCollection) {
        guard #available(iOS 13.0, *) else { return }
        
        let starClusters = rootNode.childNode(withName: "StarCluster", recursively: true)
        
        if traitCollection.userInterfaceStyle == .dark {
            directionalLight?.intensity = 900.0
            directionalLight?.color = UIColor(red: 111.0/255.0, green: 119.0/255.0, blue: 1.0, alpha: 1.0)
            directionalLight?.shadowColor = UIColor(red: 7.0/255.0, green: 2.0/255.0, blue: 36.0/255.0, alpha: 0.77)
            starClusters?.isHidden = false
            Scene.isInDarkMode = true
            configureAmbientAudio(options: .waterfalls)
        } else {
            directionalLight?.intensity = 450.0
            directionalLight?.color = UIColor(red: 1.0, green: 211.0/255.0, blue: 154.0/255.0, alpha: 1.0)
            directionalLight?.shadowColor = UIColor.black
            starClusters?.isHidden = true
            Scene.isInDarkMode = false
            configureAmbientAudio()
        }
        
        scnScene.background.contents = Asset.texture(named: "zon_bg_skyBox_a_DIFF")

        if let fog = UIColor(named: "color_fog") {
            scnScene.fogColor = fog.resolvedColor(with: traitCollection)
        }
        
        if var scenery = rootNode.childNode(withName: "Scenery", recursively: true) {
            // some scenes have scenery in the root node instead of the Scenery node
            if scenery.childNodes.count <= 2 {
                scenery = rootNode
            }
            
            for node in scenery.childNodes {
        ...
        for item in gridWorld.grid.allItems {
            if item.node.isModular {
                item.updateAppearance(traitCollection: traitCollection)
            }
        }
        
        if let waterfallTop = rootNode.childNode(withName: "zon_barrier_waterfallTop", recursively: true)?.firstGeometry?.firstMaterial {
            waterfallTop.diffuse.contents = UIImage(named: "zon_barrier_a_waterfall_DIFF")
        }
        
        if let waterfallBottom = rootNode.childNode(withName: "zon_barrier_waterfallBot", recursively: true)?.firstGeometry?.firstMaterial {
            waterfallBottom.diffuse.contents = UIImage(named: "zon_barrier_a_waterfall_DIFF")
        }
    }
}
...
extension Scene: AudioSectionPlayerDelegate {
    ...
    static func persistedAmbientAudioOptions() -> AmbientAudioOptions {
        ...
        if Persisted.areSoundEffectsEnabled {
            ...
            if !Persisted.isBackgroundAudioEnabled && !isInDarkMode {
