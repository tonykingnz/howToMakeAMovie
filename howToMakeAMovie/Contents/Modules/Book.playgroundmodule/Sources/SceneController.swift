...
final class SceneController: UIViewController {
    ...
    init(scene: Scene) {
        ...
        for device in devices {
            if device.name == "AMD Radeon Pro 5500M" || device.name == "AMD Radeon Pro 5300M" {
    ...
    func adjustAntialiasingIfNecessary() {
        #if targetEnvironment(macCatalyst)
        let kFrameTimeRunningAverageCount = 60
        let time = CFAbsoluteTimeGetCurrent()
        
        if lastRenderTime != 0.0 {
            frameTimeValues.append(time - lastRenderTime)
            
            if frameTimeValues.count == kFrameTimeRunningAverageCount {
                let runningAverageFPS = 1.0 / (frameTimeValues.reduce( 0.0, + ) / Double(kFrameTimeRunningAverageCount))
                
                if renderedFrameCount % 20 == 0 {
                    var mode = scnView.antialiasingMode

                    if runningAverageFPS > 59.0 { // upgrade if possible
                        if mode == .none {
                            mode = .multisampling2X
                        } else if mode == .multisampling2X {
                            mode = .multisampling4X
                        }
                    } else if runningAverageFPS < 40.0 { // downgrade if necessary
                        if mode == .multisampling4X {
                            mode = .multisampling2X
                        } else {
                            mode = .none
                        }
                    ...
                    if scnView.antialiasingMode != mode {
                        scnView.antialiasingMode = mode
                    }
                }
                
                frameTimeValues.removeFirst()
            }
        }
